...
references an exact git commit plus the directory path. Local runs read the
commit from the local git object database; deployed runs fetch the same commit
archive from GitHub. The resolved directory is copied into the agent's normal
skill-discovery location before the agent process starts.
"""
...
class ResolvedSkill:
    ...
    def metadata(self) -> dict[str, str | int]:
        return {
            ...
            "skill_file_count": self.file_count,
        }
...
def _validate_and_describe(root: Path, ref: SkillRef) -> ResolvedSkill:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if files > MAX_SKILL_FILES:
            raise ValueError(f"skill exceeds the {MAX_SKILL_FILES}-file limit")
        data = path.read_bytes()
        total += len(data)
        if total > MAX_SKILL_BYTES:
            raise ValueError(f"skill exceeds the {MAX_SKILL_BYTES}-byte limit")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:*") as tf:
        for member in tf:
            ...
            if member.size > MAX_SKILL_BYTES:
                raise ValueError(f"skill file is too large: {member.name}")
            extracted += 1
            total_bytes += member.size
            if extracted > MAX_SKILL_FILES:
                raise ValueError(f"skill exceeds the {MAX_SKILL_FILES}-file limit")
...
def resolve_local_git_skill(ref: SkillRef, repo_root: Path) -> Iterator[ResolvedSkill]:
    """Read a skill from an exact commit already present in a local clone."""

    check = subprocess.run(
        ["git", "-C", str(repo_root), "cat-file", "-e", f"{ref.commit}^{{commit}}"],
        capture_output=True,
        text=True,
    )
    if check.returncode != 0:
        raise ValueError(
            f"commit {ref.commit} is not available locally; fetch the branch containing it first"
        )
    archived = subprocess.run(
        ["git", "-C", str(repo_root), "archive", "--format=tar", ref.commit, ref.path],
        capture_output=True,
    )
    if archived.returncode != 0:
        raise FileNotFoundError(
            f"could not read {ref.path} at {ref.commit}: "
            f"{archived.stderr.decode(errors='replace').strip()}"
        )
    with tempfile.TemporaryDirectory(prefix="internal-ax-skill-") as tmp:
        root = Path(tmp)
        _extract_skill_archive(archived.stdout, ref, root)
...
def resolve_github_skill(ref: SkillRef) -> Iterator[ResolvedSkill]:
    ...
    if len(archive) > MAX_ARCHIVE_BYTES:
        raise ValueError(f"repository archive exceeds the {MAX_ARCHIVE_BYTES}-byte limit")
